...
namespace wstl {
    ...
    ForwardIterator
    uncheck_uninit_copy(InputIterator first, InputIterator last, ForwardIterator result, std::true_type) {
        return wstl::copy(first, last, result);
    }

    template<class InputIterator, class ForwardIterator>
    ForwardIterator
    uncheck_uninit_copy(InputIterator first, InputIterator last, ForwardIterator result, std::false_type) {
    ...
    uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator result) {   // 平凡拷贝可直接复制内存，不需调用构造函数
        return uncheck_uninit_copy(first, last, result,
    ...
    void uncheck_uninit_fill(ForwardIterator first, ForwardIterator last, const T &value, std::true_type) {
        wstl::fill(first, last, value);
    }

    template<class ForwardIterator, class T>
    void uncheck_uninit_fill(ForwardIterator first, ForwardIterator last, const T &value, std::false_type) {
    ...
    void uninitialized_fill(ForwardIterator first, ForwardIterator last, const T &value) {
        uncheck_uninit_fill(first, last, value,
                            std::is_trivially_copy_assignable<typename iterator_traits<ForwardIterator>::value_type>());
    }
}
